...
class TextPreprocessor(ABC):
    ...
    def _text_normalize(self) -> None:
        with open(PurePath(self.dpath, "comments.csv"), "r") as comments_f:
            ...
            with open(
                PurePath(self.intermediate_dpath, "comments_lower.csv"), "w"
            ) as comments_lower_f:
    ...
    def _removing_unicode(self) -> None:
        ...
        with open(
            PurePath(self.intermediate_dpath, "comments_lower.csv"), "r"
        ) as comments_f:
            commments = comments_f.readlines()
            with open(
                PurePath(self.intermediate_dpath, "comments_removed_uni.csv"), "w"
            ) as comments_rem_f:
                for comment in commments:
    ...
    def _removing_stopwords(self) -> None:
        ...
        with open(
            PurePath(self.intermediate_dpath, "comments_removed_uni.csv"), "r"
        ) as comments_f:
            commments = comments_f.readlines()
            with open(
                PurePath(self.intermediate_dpath, "comments_removed_stpwrds.csv"), "w"
            ) as comments_rem_f:
                for comment in commments:
    ...
    def _word_tokinize(self) -> None:
        tokens_list = []
        with open(
            PurePath(self.intermediate_dpath, "comments_removed_stpwrds.csv"), "r"
        ) as comments_f:
            commments = comments_f.readlines()
            with open(
                PurePath(self.prepared_dpath, "comments_tokinize.csv"), "w"
            ) as comments_rem_f:
                for comment in commments:
                    tokens = nltk.word_tokenize(comment)
                    pos_tags = nltk.pos_tag(tokens)
                    tokens_list.append([str(tag) for tag in pos_tags])
                comments_rem_f.write(str(tokens_list))
